...
let debugLiveAttempts = 0; // set to 0
let getParserParams = parser => {
  ...
  if (parser.type === 'repeat')               Object.assign(defaults, { minReps: 0, maxReps: Infinity, greedy: true });
...
let parse = (parser, input, trace) => {
  ...
  let parseNormalized = function*(parser, input, trace={ offset: 0, chain: [], debug: false }) {
    ...
    let applyParserTypeFns = {
      ...
      repeat: function*(parser, input, trace) {
        ...
        let allChildOrderings = function*(offset=0, num=0) {
          
          if (num > maxReps) return;
          
          let childTrace = { ...trace, chain: [ ...trace.chain, parser.parser ] };
          for (let parsedHead of applyParser(parser.parser, input.slice(offset), childTrace)) {
            
            if (!greedy && num >= minReps) { yield [ parsedHead ]; }
            for (let parsedTail of allChildOrderings(offset + parsedHead.result.length, num + 1)) yield [ parsedHead, ...parsedTail ];
            if (greedy && num >= minReps) yield [ parsedHead ];
      ...
      all: function*(parser, input, trace) {
        ...
        let allChildOrderings = function*(inputOffset=0, childOffset=0) {
          ...
          if (childOffset < lastChildOffset) {
            ...
            for (let parsedHead of applyParser(childParser, input.slice(inputOffset), childTrace))
            ...
            for (let parsedHead of applyParser(childParser, input.slice(inputOffset), childTrace)) yield [ parsedHead ];
      ...
      any: function*(parser, input, trace) {
        ...
        for (let cp of parser.parsers) yield* applyParser(cp, input, { ...trace, chain: [ ...trace.chain, cp ] });
        
      }
      
    };
    let applyParser0 = function*(parser, input, trace) {
      ...
      if (collapseWhiteSpace) whitespacePrefix = (input.match(globalOmitRegex) || [ '' ])[0];
      input = input.slice(whitespacePrefix.length);
      
      let doDive = function*(headParsed) {
        ...
        let diveInput = input.slice(headParsed.result.length);
        for (let diveParsed of applyParser(diveParser, diveInput, { ...trace, chain: [ ...trace.chain, diveParser ] })) {
          ...
          yield {
            ...
            result: whitespacePrefix + headParsed.result + diveParsed.result,
            
            // Maintain how the parsing looked before diving
            resultBeforeDive: whitespacePrefix + headParsed.result,
      ...
      for (let parsed of applyParserTypeFns[parser.type](parser, input, trace)) {
        ...
        let didConsume = parsed.result.length > 0;
        
        // Do a greedy dive if required
        if (diveParser && didConsume &&  diveGreedy) yield* doDive(parsed);
        
        // Return result without any diving
        yield { ...parsed, result: whitespacePrefix + parsed.result };
        
        // Do a non-greedy dive if required
        if (diveParser && didConsume && !diveGreedy) yield* doDive(parsed);
        
      }
      
    };
    let applyParser = function*(parser, input, trace) {
      
      let parsedItems = [];
      let yielded = false;
      trace.debug && console.log(`${dim('|  '.repeat(trace.chain.length))}${parser.name || 'anon'} ${dim(parser.type)}`);
      trace.debug && console.log(`${dim('|  '.repeat(trace.chain.length))}>> "${input}"`);
      for (let parsed of applyParser0(parser, input, trace)) {
        //parsed.input = input;
        trace.debug && console.log(`${dim('|  '.repeat(trace.chain.length))}<< "${parsed.result}"`);
        yielded = true;
        yield parsed;
      }
      trace.debug && !yielded && console.log(`${dim('|  '.repeat(trace.chain.length))}${dim('<<')}`);
      
    };
    
    for (let parsed of applyParser(parser, input, trace)) {
      if (input.slice(parsed.result.length).trim()) continue;
      
      //log(Infinity, simplifyParsed(parsed));
      
      yield getDenormalizedParseTree(parsed);
    }
    
  };
  return parseNormalized(getNormalizedParser(parser), input, trace);
...
(async () => {
  ...
  let genParser = () => {
    ...
    inlineValue.parsers.add({ name: 'true', type: 'token', token: 'undefined' });
    inlineValue.parsers.add({ name: 'false', type: 'token', token: 'undefined' });
    ...
    value.parsers.add({ name: 'invoke', type: 'all', parsers: [
      ...
      { name: 'delimL', type: 'token', token: '(' },
      { name: 'params', type: 'all', parsers: [
        { name: 'head', type: 'repeat', parser: { type: 'all', parsers: [ value, { type: 'token', token: ',' } ] } },
        { name: 'tail', type: 'repeat', maxReps: 1, parser: value }
      ]},
      { name: 'delimR', type: 'token', token: ')' }
  ...
  let parsed = parse(genParser(), input, { offset: 0, chain: [], debug: false }).next().value;
