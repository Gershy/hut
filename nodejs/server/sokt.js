...
module.exports = getRoom('setup.hut.hinterland.RoadAuthority').then(RoadAuthority => {
  ...
  return form({ name: 'SoktRoadAuthority', has: { RoadAuthority }, props: (forms, Form) => ({
    ...
    $SoktRoad: form({ name: 'SoktRoad', has: { Road: RoadAuthority.Road }, props: (forms, Form) => ({
      ...
      $wsEncode: ({ op=1, code=null, text=null, data=Buffer.from(text ?? '', 'utf8') }) => {
        ...
        let len = data.length; // Note Buffer.byteLength??
      ...
      wsIncoming(ms, buff) {
        ...
        while (this.onn()) {
          ...
          let { consumed, ...params } = wsMsg;
          
          this.buff = this.buff.slice(consumed);
          this.size -= consumed;
          
          let { fin, op, mask, data } = params;
          
          // TODO: HEEERE copied from `wsWriteQueue` below - also lifted `wsEncode` and `wsDecode`
          // up! Everything is ALMOST working - just run chess2 and debug!!
          gsc({ ms, incoming: data.toString('utf8') });
      ...
      wsWrite(opts /* { op, code, text, data } */) {
        
        let err = Error('');
        return this.writeQueue = this.writeQueue.then(() => Promise(rsv => {
        
          socket.write(Form.wsEncode(opts), cause => {
            if (cause) {
              cause.suppress();
              this.end();
